...
/*
async function cekIzinLokasi() {
    if (!navigator.permissions || !navigator.geolocation) {
        alert("Browser tidak mendukung Permissions API atau Geolocation API");
        return {
            status  : false,
            msg     : "Browser tidak mendukung Permissions API atau Geolocation API"
        }
    }

    try {
        const status = await navigator.permissions.query({ name: 'geolocation' });
        console.log("Status izin lokasi:", status.state);

        if (status.state === 'granted') {
            console.log("Izin sudah diberikan");
            return {
                status  : "granted",
                msg     : "Izin sudah diberikan"
            }
        } else if (status.state === 'prompt') {
            console.log("Belum ada izin, meminta sekarang...");
            return {
                status  : "load",
                msg     : "Belum ada izin, meminta sekarang..."
            }
        } else if (status.state === 'denied') {
            alert("Izin lokasi ditolak. Silakan aktifkan manual di browser.");
            return {
                status  : "denied",
                msg     : "Izin lokasi ditolak. Silakan aktifkan manual di browser."
            }
        }

        status.onchange = () => {
          console.log("Perubahan izin lokasi:", status.state);
        };

    } catch (err) {
        console.error("Gagal cek izin lokasi:", err);
        return {
            status  : false,
            msg     : "Gagal cek izin lokasi:", err
        }
    }
}
//cekIzinLokasi()
function getPosisi() {
    navigator.geolocation.getCurrentPosition(
        (pos) => {
            console.log("Lokasi didapat:", pos.coords.latitude, pos.coords.longitude);
            alert(`Lokasi: ${pos.coords.latitude}, ${pos.coords.longitude}`);
        },
        (err) => {
